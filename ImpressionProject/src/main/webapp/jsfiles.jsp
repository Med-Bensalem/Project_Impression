     <script src="${pageContext.request.contextPath}/assets/libs/@popperjs/core/dist/umd/popper.min.js"></script>
    <script src="${pageContext.request.contextPath}/assets/libs/bootstrap/dist/js/bootstrap.min.js"></script>
    <script src="${pageContext.request.contextPath}/assets/libs/simplebar/dist/simplebar.min.js"></script>
    <!-- Include theme JS -->
    <script src="${pageContext.request.contextPath}/assets/js/theme.min.js"></script>
    <!-- Include validation JS -->
    <script src="${pageContext.request.contextPath}/assets/js/vendors/validation.js"></script>
    
       <script src="${pageContext.request.contextPath}/assets/libs/tiny-slider/dist/min/tiny-slider.js"></script>
        <script src="${pageContext.request.contextPath}/assets/js/vendors/tnsSlider.js"></script>
        <script src="${pageContext.request.contextPath}/assets/libs/glightbox/dist/js/glightbox.min.js"></script>
        <script src="${pageContext.request.contextPath}/assets/js/vendors/glight.js"></script>
        
   